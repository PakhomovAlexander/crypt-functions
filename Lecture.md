...
1) A и B выбирают достаточно большие простые числа A(**p1, p2**), B(**q1,q2**), 
пемножают **rA = p1p2**, **rB = q1q2** и вычисляют **ф(rA)**, **ф(rB)**
2) Алиса выбирает число **1 < a < ф(rA)**, **gcd(a, ф(rA))=1**, 
Боб выбирает число **1 < b < ф(rB)**, **gcd(b, ф(rB))=1**
3) Печатается абоненская книга, rA, a и rB, b
4) Вычисление секретных ключей. **alpha** - секретный ключ Алисы, **beta** - Боба. 
**alpha * a == 1 (mod(ф(rA))), beta * b == 1 (mod(ф(rB)))**

### Как это работает? 
1) Алиса хочет передать секретное сообщение m Бобу. **gcd(m, rB) = 1** (если m < чем q1 и m < q2) 
Алиса вычисляет: **m1 == m^b (mod rB)**, m1 - шифротекст, он передается Бобу.
2) Боб вычисляет **m2 == m1^beta (mod rB)**. Оказывается, m2 = m. 
...
### Примеры атак на RSA 
#### Метод безключевого счтения
Предпологаем что криптоаналитику известны 
1) rA, a 
2) c 

Криптоаналитик перешифровывает шифротекст c, с помощью rA еще k раз, до тех пор, пока снова не получит 
тот же самый шифротекст **c**, (c^a)^k == c (mod rA), gcd(c, rA) = 1, <br>
c^(ф(rA)) == 1 (mod rA) | (*c) <br>
c^(ф(rA)+1) == c (mod rA)  <br>
a * k = ф(rA)+1    ? <br>
c^k = (m^a)^k = m^(ф(rA)+1)  = m^ф(rA) * m = m   в ZrA 

#### Атака на базе общего модуля 

Предположим, что криптосистема RSA построена на основве общего модуля, имеется ряд абонентов: 
A(rA, a, alpha), B(rA, b, beta), C(rC, c, gamma).   <br>
A шифрует m1 -> B зразшифровывает m1, а потом зашифровывает его в m2 -> C <br>
В этой ситуации криптоаналитику известны :  rA, a, b, m1, m2 <br>
Предположим: gcd(a, b) = 1 <br>
Тогда: m1^x * m2^y = (m^a)^x * m^b)^y = m^(ax+by) = m <br>
ac + by = 1  - решаем это уравнение <br>
x0, y0 - частные решения (*), тогда <br>
m1^x0 * m2^y0 = m^(ax0 + by0) = m

### Задача 
Криптосистема с общим модулем: cd RSA файл 

## Протоколы аутентификации 
Суть протоколов аутентификации состои в подтверждении информации. 

### Протокол Schnorr 

1) А и В выбирают числа **p, q ^ (p-1)** делится на **q**, **g** из Zp, **g^q == 1 (mod p), g != 1**
2) Алиса выбирает **k**, a из {1,...,q-1}  <br>
**y == g^-k в Zp** y*g^k == 1 в Zp <br>
**r == g^a в Zp**  <br>

(y,r) -> Бобу

Процедура аутентификации, проверяется подленность Алисы:
1) Боб выбирает случайное чило **e** и направляет Алисе
2) Алиса вычисляет **s == a + k * e (mod q)**
3) Боб проверяет соотношение **r == g^s * y^e (mod p)**, если да, то Боб принимает доказательство Алисы.

#### Задача 
Привести обоснование соотношения.

### 
Задача 


### Электронная подпись на основе системы МАЛЯ? 
А отправляет, В принимает и хочет убедится, что это сообщение именно от А 

A) формирование открытого ключа 
1) А выбирает достаточно большое р 
2) Выбирает g из поля Zp, g – примитивный элемент 
3) Выбирается х {1, p-1} – секретный ключ 
4) Вычисляет y = gx(mod p) (p,g,y) – open key

B) 
1) k gcd(k, p-1) = 1
2) r == g^k(mod p)
3) s == (Q-xr)k^-1 (mod p-1)

C) Провекра эл. подписи
1) 0 <= r < p, 0 <= s < p-1
2) y^r * y^s -- g^Q (mod p) ( * )

Если * выполнена то аблнент Б подтверждает, иначе отвергает


### Задача на дом 
Известен открытый ключ (p,g,y) = (337, 15, 303)
1) Q = 309 (r,s) = (31, 232)
2) Q = 19 (r,s) = (31, 74)
3) Q = 106 (r,s) = (31, 324)
4) Q = 106 (r,s) = (185, 81)
